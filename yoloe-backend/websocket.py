...
class YoloWebSocketServer:
    ...
    async def broadcast_annotated_frame(self, annotated_jpeg_bytes: bytes, annotations: list, detection_data=None):
        ...
        payload = {
            ...
            "timestamp": detection_data.get("timestamp") if detection_data else None,
            "current_prompts": detection_data.get("current_prompts", []) if detection_data else [],
            "model_type": "YOLO-E"
        }
        ...
        if detection_data:
            payload.update({
                "motor_data": detection_data.get("motor_data", {}),
                "frame_timestamp": detection_data.get("frame_timestamp"),
                "detection_timestamp": detection_data.get("detection_timestamp"),
                "image_shape": detection_data.get("image_shape")
            })
